...
def get_consSeq(ref, bam, bed, cov, out, done):
    ...
    touch {done}
    """.format(ref = ref, bam = bam, bed = bed, cov1=str(float(cov)/3), cov2=str(2*float(cov)),
               out = out, done = done)

    return (inputs, outputs, options, spec)


def make_input(inp, out, path, done):
    """Make psmc input."""
    inputs = [path+inp]
    outputs = [path+out,path+done]
    ...
    touch {done}
    """.format(inp = path+inp, out = path+out, done = path+done)

    return (inputs, outputs, options, spec)

def run_psmc(inp, out, p,  path, done):
    """Run psmc."""
    inputs = [path+inp]
    outputs = [path+done]
    ...
    touch {done}
    """.format(inp = path+inp, out = path+out, p = p, done = path+done)
